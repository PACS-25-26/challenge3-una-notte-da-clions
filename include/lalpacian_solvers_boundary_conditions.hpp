...
void apply_dirichlet_condition(eigenMatrix & u_h, const Data_Struct<funcType>& data, const eigenMatrix & meshX, const eigenMatrix & meshY, const unsigned mpi_rank, const unsigned mpi_size){
    ...
    if constexpr(execution_mode == ExecutionMode::SEQUENTIAL){
        ...
        // Bottom edge
        if(rank == 0) for(unsigned i = 0; i < data.n; i++) u_h(last, i) = data.f1(meshX(last, i), meshY(last, i));
        
        // Top edge
        if(rank == mpi_size - 1) for(unsigned i = 0; i < data.n; i++) u_h(0, i) = data.f3(meshX(0, i), meshY(0, i));
...
/**
     ...
     */
template <BoundaryCondition boundary_condition, ExecutionMode execution_mode, typename funcType>
void apply_neumann_condition(eigenMatrix & u_h, const Data_Struct<funcType>& data, const eigenMatrix & meshX, const eigenMatrix & meshY, const unsigned mpi_rank){
    ...
    if constexpr(execution_mode == ExecutionMode::SEQUENTIAL){
         ...
         for (unsigned i = 0; i < data.n; i++) {
                ...
                u_new(last, i) = u_new(last - 1, i) + h * data.f1(meshX(last, i), meshY(last, i)); // bottom edge 
            }
        ...
        // Bottom edge
        if(rank == 0) for(unsigned i = 0; i < data.n; i++) u_h(last, i) = u_h(last - 1, i) + h * data.f1(meshX(last, i), meshY(last, i));
        
        // Top edge
        if(rank == mpi_size - 1) for(unsigned i = 0; i < data.n; i++) u_h(0, i) = u_h(1, i) + h * data.f3(meshX(0, i), meshY(0, i));
...
template <BoundaryCondition boundary_condition, ExecutionMode execution_mode, typename funcType>
void apply_robin_condition(eigenMatrix & u_h, const Data_Struct<funcType>& data, const eigenMatrix & meshX, const eigenMatrix& meshY, const unsigned mpi_rank){
    const unsigned last = data.n - 1;
    const double h = abs(data.x2 - data.x1) / (data.n - 1);
    ...
    if constexpr(execution_mode == ExecutionMode::PARALLEL){
        ...
        // Bottom edge
        if(rank == 0) for(unsigned i = 0; i < data.n; i++) u_h(last, i) = (u_h(last - 1, i) + h * data.f1(meshX(last, i), meshY(last, i))) / den;
        
        // Top edge
        if(rank == mpi_size - 1) for(unsigned i = 0; i < data.n; i++) u_h(0, i) = (u_h(1, i) + h * data.f3(meshX(0, i), meshY(0, i))) / den;
